...
if ($json_data->type == 'TopRetailer') {
	...
	if ($state_id != '') {
		$state_between = "AND employees__state.state_token ='$state_id'";
	} else {
		$state_between == '';
	}
	if ($year_month != '') {
	...
	$date = "AND YEAR(`orders`.`date_time`)='$year'";
	$state_between='';
}
...
WHERE 
`orders`.`order_type` IN('Sales Order','Spot Order','Retailer order') AND `orders`.`delivery` = 'Completed'  $state_between $date
GROUP BY
`orders`.`shop_token`
ORDER BY
`billing_amt`
