...
INNER JOIN orders ON orders.shop_token = shop_mapping.token
WHERE
 sales_emp_token = '$employee_id' AND units__shop_mapping.delete_status = 1 AND date(orders.`date_time`) LIKE '$dateonly%' AND   daily_schedule.schedule_date = '$dayname' AND orders.delivery='Pending'
GROUP BY
 shop.name");                           
