...
$employee_id = $json_input->employee_id;//distributor_token
$shop_id = $json_input->shop_id;
$currnetDate =  date("Y-m-d");

$shoptoken = mysqli_query($link,"SELECT token FROM shop_mapping WHERE `shop_token` = '$shop_id'");
$shopToken = mysqli_fetch_array($shoptoken);
$shop_token = $shopToken['token'];

$shop_unit = mysqli_query($link,"SELECT unit_group_token FROM units__shop_mapping WHERE `shop_token` = '$shop_token' AND `distributor_token`='$employee_id'");
$shop_units = mysqli_fetch_array($shop_unit);
$unitToken = $shop_units['unit_group_token'];
if($unitToken!=""){
$shop_query = mysqli_query($link,"SELECT
                    ...
                    INNER JOIN units__shop_mapping ON units__shop_mapping.shop_token = shop_mapping.token
                    INNER JOIN units ON units__shop_mapping.unit_group_token=units.token
                    LEFT JOIN orders ON orders.shop_token = units__shop_mapping.shop_token and orders.delivery != 'Cancelled'
                    INNER JOIN employees ON employees.token = shop_mapping.distributor_token
                    WHERE
                        shop.token = '$shop_id'  AND shop_mapping.distributor_token = '$employee_id'  GROUP BY orders.shop_token");
...
INNER JOIN shop__type ON shop.shop_type_code = shop__type.token
INNER JOIN shop__outstanding ON shop_mapping.token = shop__outstanding.shop_token
LEFT JOIN orders ON orders.shop_token = shop_mapping.token AND orders.delivery != 'Cancelled'
INNER JOIN employees ON employees.token = shop_mapping.distributor_token
WHERE
    shop.token = '$shop_id' AND shop_mapping.distributor_token = '$employee_id'
GROUP BY
    orders.shop_token");
}
    ...
    $shop_row = mysqli_fetch_array($shop_query);
        ...
        $bill_discount_percentage = $shop_row['bill_discount_percentage'];
         ...
         if($bill_discount_amount > 0) {
                    ...
                    $shop_row['shop_name'] == null ? $shop_obj->name= '' : $shop_obj->name = $shop_row['shop_name'];
                    $shop_row['city'] == null ? $shop_obj->city= '' : $shop_obj->city = $shop_row['city'];
                    $shop_row['address'] == null ? $shop_obj->address= '' : $shop_obj->address = $shop_row['address'];
                    $shop_row['shop_type'] == null ? $shop_obj->shop_type= '' : $shop_obj->shop_type = $shop_row['shop_type'];
                    $shop_row['mobile_number'] == null ? $shop_obj->mobile_number= '' : $shop_obj->mobile_number = $shop_row['mobile_number'];
                    $shop_row['unit_name'] == null ? $shop_obj->unit_name= '' : $shop_obj->unit_name = $shop_row['unit_name'];
                    $shop_row['shop_show_status'] == null ? $shop_obj->shop_status= '' : $shop_obj->shop_status = $shop_row['shop_show_status'];
                    $shop_row['name'] == null ? $shop_obj->distributor_name= '' : $shop_obj->distributor_name = $shop_row['name'];
                    $shop_row['token'] == null ? $shop_obj->distributor_token= '' : $shop_obj->distributor_token = $shop_row['token'];
