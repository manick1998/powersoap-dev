...
$currentDate =  date('Y-m-d');
                    ...
                    `shop`.`city`,
                    `shop`.`pincode`,
                    COALESCE(`shop_mapping`.`unit_token`, 0) AS unit_token,
                    shop__type.name AS shop_type,
                    COALESCE(units.name,'No Unit') AS unit_name
                    FROM
                    `employees`
                    INNER JOIN shop_mapping ON shop_mapping.distributor_token = `employees`.`token` AND shop_mapping.status='1'
...
while($row = mysqli_fetch_array($sql)){
    $obj = new stdClass; 
    $shopToken = $row['token'];
    $query = mysqli_query($link,"SELECT `token` FROM `shop_mapping` WHERE `shop_token`='$shopToken' AND `distributor_token`='$distributor_token'");
    $row1 = mysqli_fetch_array($query);
    $shop_mapToken = $row1['token'];
    $sql2 = mysqli_query($link,"SELECT
                            ...
                            `orders`
                            WHERE
                            shop_token = '$shop_mapToken' AND date(`date_time`)='$currentDate'
                            ORDER BY
                            id
                            DESC
                            LIMIT 1");
    ...
    $order_status = $row2['delivery']==""?"":$row2['delivery'];
    $timestamp = $get_lastDate;
    $splitTimeStamp = current(explode(" ",$timestamp));
   if($splitTimeStamp == $currentDate && $shop_mapToken!=""){
