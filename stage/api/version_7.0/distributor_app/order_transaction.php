    ...
    if($order_select){
         ...
         $new_amt = $paid_amount+$amount;
            if($bill_amt >= $new_amt){
                 ...
                 $inser_amt = mysqli_query($link,"INSERT INTO `shop__order_transaction`( `date_time`,`order_token`, `shop_token`, `amount`, `payment_mode`, `employee_id`) VALUES ('$indiaDateTime_Val','$order_id','$shop_token','$amount','$payment_type','$distributor_token')");
            }
    }
$obj = new stdClass;
if($inser_amt){
