...
function login(){
        $query = "SELECT `mobile_number` FROM `shop` WHERE `mobile_number`=? AND `shop_show_status`='Active'";
    ...
    function distributor_orders_details(){
        ...
        COALESCE(orders.token,0) AS orders_id,
      ...
      COALESCE(orders.outstanding_amount,0)AS outstanding_amount,
        orders.delivery,
        shop.address,
        COALESCE(orders.date_time,0) AS schedule_date
    FROM
        shop
    ...
    LEFT JOIN orders ON  orders.shop_token =shop_mapping.token 
    ...
    INNER JOIN shop__outstanding ON shop__outstanding.shop_token = shop_mapping.token
    ...
    FROM
        `shop_mapping`
    INNER JOIN orders ON orders.shop_token = shop_mapping.token
    INNER JOIN shop__outstanding ON shop__outstanding.shop_token = shop_mapping.token
    WHERE
        orders.token = ?";
    ...
    function orderval($shop_mapping_token){
        $query="SELECT `token` FROM orders  WHERE `shop_token` = '$shop_mapping_token' ORDER BY id DESC LIMIT 1";
        $stmt=$this->conn->prepare($query);
    ...
    //state and region
    function stateList(){
        $query = "SELECT `state_token`,`state_name` FROM `employees__state`";
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
        return $stmt;
    }
    function regionList(){
        $query = "SELECT `token`,`region_name`,`state_id` FROM `region`";
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
        return $stmt;
    }
function shopType(){
    $query = "SELECT `token`,`name` FROM `shop__type`";
    $stmt = $this->conn->prepare($query);
    $stmt->execute();
    return $stmt;
}
function divisionList(){
    $query = "SELECT `token`,`name` FROM `products__category` WHERE `delete_status`='1'";
    $stmt = $this->conn->prepare($query);
    $stmt->execute();
    return $stmt;
}
