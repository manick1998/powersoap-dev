...
$order_insert = mysqli_query($link,"INSERT INTO `orders`( `token`,`order_number`, `date_time`, `order_type`, `shop_token`, `employee_token`,`distributor_token`, `items`, `billing_amount`,`outstanding_amount`,`delivery`,`delivery_emp_token`, `approved_on`, `delivered_on`,`unit_shop_count`) 
                        VALUES ('$token','$order_number', '$currnetDateTime','$order_type','$shop_token','$sale_id','$dist_token_value' ,'$final_product_item_count','$bill_amount','0','Completed','$sale_id','$currnetDateTime','$currnetDateTime','$unit_shop_count')");
...
$check = mysqli_num_rows($check_shop);
            ...
            if($check) {
                        ...
                        $check_status = mysqli_query($link,"SELECT 1 FROM `sales__log` WHERE `distributor_token`='$dist_token_value' AND `sales_token`='$sale_id' AND  `shop_token` ='$shop_token' AND `date_time` LIKE '%$indiaDate%'");
