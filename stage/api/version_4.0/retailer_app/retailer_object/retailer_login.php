...
function login(){
    ...
    FROM
        `shop`
    INNER JOIN employees ON shop.distributor_token = employees.token
    WHERE
        shop.mobile_number = ?";
...
$stmt = $this->conn->prepare( $query );
    ...
    WHERE
        shop.mobile_number = ? AND shop.distributor_token = ?  AND shop.delete_status = 1
    GROUP BY
       orders.token
       ORDER BY
        orders.id
    ...
    WHERE
        employees__division_mapping.employee_token = ? AND employees__division_mapping.delete_status = '1' AND products.delete_status = 1
    GROUP BY
        products.token";
        $stmt = $this->conn->prepare($query);
