...
// if($order_row['units'] == 'Box'){
    $order_obj->amount = number_format($order_row['price_per_unit']*$order_row['qty']/1.18, 2, '.', '');
// }else{
  ...
  array_push($array,$order_obj);
    ...
    $order_total = mysqli_query($link,"SELECT
                            -- SUM( orders__items.misc_price * orders__items.quantity) AS gst,
                             SUM( orders__items.misc_price ) AS gst,
            ...
            $gst_cal = ($order_mount['billing_amount']/1.18*0.18);
            ...
            $value = round($order_mount['billing_amount']/1.18);
