...
$order_number = "ORD-R".$token;
   ...
   $getShopDistributor = mysqli_query($link,"SELECT `distributor_token` FROM `shop_mapping` WHERE `shop_token` = $shop_token");
...
VALUES ('$token','$order_number','$currnetDateTime','$order_type','$shop_token','$shop_distributor_token','$shop_distributor_token','$final_product_item_count',0,'$bill_amount','pending','$unit_shop_count','1','$sale_id')");
 ...
 if($order_insert) {
    ...
    $check_status = mysqli_query($link,"SELECT * FROM `sales__log` WHERE `distributor_token`='$dist_token_value' AND `sales_token`='$sale_id' AND  `shop_token` ='$shop_token' and `date_time` LIKE '$indiaDate%'");
    if(mysqli_num_rows($check_status) >0){

                 $sales_update = mysqli_query($link,"UPDATE `sales__log` SET `status`='Completed'  WHERE `distributor_token` = '$dist_token_value' AND `sales_token` = '$sale_id' AND `shop_token`='$shop_token' AND `date_time` LIKE '$indiaDate%'");
    }
        else
        {
        $sales_log = mysqli_query($link,"INSERT INTO `sales__log`(`date_time`, `distributor_token`, `sales_token`, `shop_token`, `status`) VALUES ('$currnetDateTime','$dist_token_value','$sale_id','$shop_token','Completed')");
        }
}
