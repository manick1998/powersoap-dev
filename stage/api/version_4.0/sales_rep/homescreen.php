...
`area`.`area_name`,
COUNT(DISTINCT `shop`.`id`) AS `shop_count`,
...
LEFT JOIN `orders` ON `orders`.`shop_token` = `shop`.`token` AND YEAR(orders.date_time) = YEAR(CURRENT_DATE()) AND MONTH(`orders`.`date_time`) = MONTH(CURRENT_DATE())
WHERE
`schedule_sales_rep`.`sales_rep_token` = '$sales_rep_employee_token' AND `employees`.`deparment_token` = '18028120' AND `schedule_sales_rep`.`schedule_date` = '$currentDate' AND `schedule_sales_rep`.`status` = '1'
GROUP BY
`employees`.`token`");
