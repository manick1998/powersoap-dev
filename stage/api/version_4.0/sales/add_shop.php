...
//get state
$get_State_id = mysqli_query($link,"SELECT `state_id` FROM `employees` where `token` = '$ditributor_id'");
...
$insert_shop = mysqli_query($link,"INSERT INTO `shop`( `token`, `name`, `date_time`, `unit_token`,  `retail_code`, `shop_type_code`, `slot`, `distributor_token`, `mobile_number`,
							 `contact_person`, `join_date`, `license_number`,license_image,  `delete_status`, `address`, `city`, `state_id`, `pincode`, `coordinates`,`created_by`) VALUES ('$token','$shop_name','$date','$units_id' , '$retailcode','$shop_type','0%','$ditributor_id','$contact_number','$contact_name','$date','$licence_number','$upload_url', '1','$address','$city','$state_id','$pincode','$coordinates','$employee_id')");
