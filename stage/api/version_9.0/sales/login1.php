...
if ($device_token != '') {
    $query = mysqli_query($link, "UPDATE `employees` SET `device_token`='$device_token' WHERE `mobile_number`='$mobile'");
}
...
if ($check_status  > 0) {
    if ($password == 'sales@123' && $password_check > 0) {
        ...
        $update = mysqli_query($link, "UPDATE `employees` SET `password`='$passwords' WHERE `mobile_number`='$mobile' AND deparment_token= '45916684'");
