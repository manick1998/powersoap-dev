...
if ($searchText == '') {
  ...
  $searchQuery = "AND shop.name like '%$searchText%'";
}
...
INNER JOIN `shop_mapping` ON `shop_mapping`.`unit_token`=`unit_customise_mapping`.`unit_token`
INNER JOIN `shop` ON `shop`.`token` = `shop_mapping`.`shop_token`
LEFT JOIN `orders` ON `orders`.`shop_token` =`shop_mapping`.`token` AND orders.date_time LIKE '$dateonly%' and orders.delivery != 'Cancelled' AND employee_token ='$employee_id'
WHERE
  ...
  // //last index
  // $get_lastIndex = mysqli_query($link,"SELECT MIN(shop.id)as last_index FROM `shop` INNER JOIN shop_mapping ON shop.token=shop_mapping.shop_token INNER JOIN custom_daily_schedule ON custom_daily_schedule.unit_token=shop_mapping.unit_token INNER JOIN unit_customise_mapping on unit_customise_mapping.distributor_token=shop_mapping.distributor_token WHERE shop_mapping.distributor_token='$distributor_id' AND custom_daily_schedule.sales_emp_token='$employee_id' AND custom_daily_schedule.schedule_date='$dayname'");
  // $row_last = mysqli_fetch_array($get_lastIndex);
  // $lastindex = $row_last['last_index'];



  //shop count
  $shop_data = mysqli_query($link, "SELECT count(shop.token)as shop_count FROM unit_customise_mapping 
INNER JOIN `shop_mapping` ON unit_customise_mapping.unit_token = shop_mapping.unit_token
...
INNER JOIN `shop` as `shopData` ON `shopData`.`token` = `shop_mapping`.`shop_token`
...
GROUP BY shop.token");
  ...
  //visted
  $shop_visted = mysqli_query($link, "SELECT * FROM `salesman_shop_visited` WHERE `employee_token`='$employee_id' AND `status`='0' AND `date`='$indiaDate'");
  ...
  $shopVisted_count = mysqli_num_rows($shop_visted);
  $check_countvalue_data = $amt_value1['shop_count_value'];


  $obj_data = new stdClass;
  $obj_data->cover_today_value = $shoptake_value_12 + $shopVisted_count;
  $shop_total1 = $shoptake_value_12 + $shopVisted_count;
  ...
  $obj_data->achived_productivity = $shop_total1;
  ...
  if ($check_countvalue_data > 0) {
    $obj_data->productivity = round(($shop_total1 / $amt_value1['shop_count_value']) * 5);
  } else {
...
WHERE
  custom_daily_schedule.sales_emp_token = '$employee_id' AND custom_daily_schedule.schedule_date = '$dayname' AND shop.delete_status = '1'  AND unit_customise_mapping.delete_status = '1'  $shopQuery $searchQuery
GROUP BY
   shop.token
ORDER BY
  shop.id
...
LEFT JOIN `orders` ON `shop_mapping`.`token` = `orders`.`shop_token`
INNER JOIN `shop` ON `shop`.`token` = `shop_mapping`.`shop_token`
WHERE
...
sales_emp_token = '$employee_id' AND units__shop_mapping.delete_status = 1 AND daily_schedule.schedule_date = '$dayname' AND delivery in('Pending','Completed') GROUP BY orders.shop_token");
  ...
  $shoptake_value_12 = mysqli_num_rows($shop_value1);
  $shopVisted_count = mysqli_num_rows($shop_visted);
  ...
  $obj_data = new stdClass;
  $obj_data->cover_today_value = $shoptake_value_12 + $shopVisted_count; //intval($shoptake_value['count_value1']) ;
  $shop_total1 = $shoptake_value_12 + $shopVisted_count;
  ...
  $obj_data->achived_productivity = $shop_total1; //intval($shoptake_value['count_value1']);
  ...
  if ($check_countvalue > 0) {
    $obj_data->productivity = round(($shop_total1 / $amt_value['count_value']) * 5);
  } else {
  ...
  // //last index
  // $get_lastIndex = mysqli_query($link,"SELECT MIN(shop.id)as last_index FROM `shop` INNER JOIN shop_mapping ON shop.token=shop_mapping.shop_token INNER JOIN daily_schedule ON daily_schedule.unit_token=shop_mapping.unit_token INNER JOIN units__shop_mapping on units__shop_mapping.distributor_token=shop_mapping.distributor_token WHERE shop_mapping.distributor_token='$distributor_id' AND daily_schedule.sales_emp_token='$employee_id' AND daily_schedule.schedule_date='$dayname'");
  // $row_last = mysqli_fetch_array($get_lastIndex);
  // $lastindex = $row_last['last_index'];
  ...
  if ($searchText == '') {
    $shop_query = mysqli_query($link, "SELECT shop.id,
                        ...
                        WHERE
                            sales_emp_token = $employee_id AND units__shop_mapping.delete_status =1 AND   daily_schedule.schedule_date='$dayname' $shopQuery $searchQuery GROUP BY shop.token ORDER BY shop.id Desc Limit 10");
  } else {
    $shop_query = mysqli_query($link, "SELECT shop.id,
                           shop.name,
                           shop.token,
                        ...
                        INNER JOIN units ON units.token = daily_schedule.unit_token
                        WHERE
                            sales_emp_token = $employee_id AND units__shop_mapping.delete_status =1 AND   daily_schedule.schedule_date='$dayname' $shopQuery $searchQuery GROUP BY shop.token");
  }
