...
$sql1 = mysqli_query($link,"SELECT
			    ...
			    WHERE `delete_status` = '1' AND `employees`.`deparment_token` = '18028120' AND `employees`.`token`=$employee_id and orders.order_type='Distributor Order' ORDER BY orders.id DESC");
                while($distributor_data = mysqli_fetch_array($sql1)){
                    ...
                    $obj->invoice_name = $estimate_url.$distributor_data['invoice_name'];
