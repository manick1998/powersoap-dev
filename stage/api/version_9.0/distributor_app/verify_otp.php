...
$obj_emp=new stdClass;
 $obj=new stdClass;
if($result1) {
    $sql = mysqli_query($link,"SELECT employees.`id`, employees.`token`, employees.`name`, employees.`employees_code`, employees.`deparment_token`, employees.`gender`, employees.`mobile_number`, deparment.name as dept_name FROM `employees` INNER JOIN deparment ON deparment.token = employees.deparment_token WHERE employees.mobile_number= $mobile AND deparment.token= '18028120'" );
