...
if($employee_id){
$insert_shop = mysqli_query($link,"UPDATE `shop` SET `name`='$shop_name',`date_time`='$date',`shop_type_code`='$shop_type',`distributor_token`='$ditributor_id',`mobile_number`='$contact_number',`contact_person`='$contact_name',`license_number`='$licence_number',`license_image`= '$upload_url',`address`='$address',`city`= '$city',`pincode`='$pincode',`coordinates`='$coordinates'  WHERE `token` ='$shop_id' ");
