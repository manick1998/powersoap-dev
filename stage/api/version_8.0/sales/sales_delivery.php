...
if (mysqli_num_rows($check) > 0) {
    ...
    $update_query = mysqli_query($link, "UPDATE `orders` SET `delivery`= 'Completed',delivered_on='$currnetDateTime' ,`delivery_emp_token`='$employee_id' WHERE `shop_token` = '$shop_id' AND `token`= '$order_id' AND delivery != 'Cancelled' ");
    ...
    //sales rep order
    $checkSalesRepOrder = mysqli_query($link, "SELECT `order_number`, `sales_rep_token` FROM `orders` WHERE  `shop_token`= '$shop_id' AND `token`= '$order_id' AND `delivery`= 'Completed' AND `is_slaes_rep_admin` = '1'");
