...
$obj = new stdClass;
$check_status = mysqli_query($link,"SELECT * FROM `sales__log` WHERE `distributor_token`='$dist_token_value' AND `sales_token`='$employee_id' AND  `shop_token` ='$shop_mapping_token'");
    if($row_check = mysqli_num_rows($check_status) == 0){
      ...
      $insert_log = mysqli_query($link,"INSERT INTO `sales__log`( `date_time`, `distributor_token`, `sales_token`, `shop_token`, `status`, `reason`) VALUES ('$date','$dist_token_value',
      	'$employee_id','$shop_mapping_token','Pending','$reason')");
      $obj->status_code=200; 
