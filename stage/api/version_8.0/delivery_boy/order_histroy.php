...
WHERE
shop.token = '$shop_id' and shop_mapping.distributor_token = '$distributor_token'  and orders.delivery != 'Cancelled'");
$get_outstanding_amt = mysqli_fetch_array($outstanding_amount);


 $bill_discount_amount = $get_outstanding_amt['bill_discount_amount'];
  $bill_discount_percentage = $get_outstanding_amt['percentage_value'];
...
INNER JOIN shop ON shop.token = shop_mapping.shop_token
WHERE
shop.token = '$shop_id' and shop_mapping.distributor_token = '$distributor_token' ORDER BY orders.id DESC");
...
while($order_row = mysqli_fetch_array($order_hisyory)) {
    $obj_order = new stdClass;
        ...
        $obj_order->delivery = $order_row['delivery'];
