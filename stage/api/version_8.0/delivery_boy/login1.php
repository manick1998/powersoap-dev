...
$sql1 = mysqli_query($link,"SELECT
                           mobile_number
                            FROM
                                `employees`
                            INNER JOIN deparment ON deparment.token=employees.deparment_token WHERE employees.`mobile_number` ='$mobile' AND employees.block_status = '1' AND deparment.token= 93402780");
...
if($check_status > 0) {
if($passwords=='del@123' && $password_check > 0) {
    $password1 = 123;
    $password2 = 'del';
    ...
    $password4 = hash('sha512', $passwords);
