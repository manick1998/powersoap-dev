...
//unit_token
$unit = mysqli_query($link,"SELECT DISTINCT `unit_token` FROM `daily_schedule` WHERE date(date_time)='$dateonly' AND `delivery_emp_token`='$employee_id'");
$unit_row = mysqli_fetch_array($unit);
$unit_token = $unit_row['unit_token'];
...
//echo $count;
if ($count == 0) {
$overallCount  = mysqli_query($link,"SELECT DISTINCT orders.token FROM orders INNER JOIN shop_mapping ON shop_mapping.token =orders.shop_token INNER JOIN unit_customise_mapping ON unit_customise_mapping.unit_token=shop_mapping.unit_token WHERE unit_customise_mapping.custom_unit_token='$unit_token' AND date(orders.date_time)='$dateonly' GROUP BY orders.shop_token");
                            
                            $overallCount_row = mysqli_num_rows($overallCount);
//order logs statuscheck 

             $shop_value1 = mysqli_query($link,"SELECT DISTINCT orders.token FROM orders INNER JOIN shop_mapping ON shop_mapping.token =orders.shop_token INNER JOIN unit_customise_mapping ON unit_customise_mapping.unit_token=shop_mapping.unit_token WHERE unit_customise_mapping.custom_unit_token='$unit_token' AND date(orders.date_time)='$dateonly'AND orders.delivery='Completed' GROUP BY orders.shop_token");
...
$obj_data->cover_today_outoff= intval($overallCount_row);//intval($amt_value['pending_count']+$amt_value['Completed_count']);
...
$obj_data->overall_productivity = intval($overallCount_row);//5;
...
if($shoptake_value12 > 0){
    if($overallCount_row ==0){
    $obj_data->overall =0;// round(($shoptake_value12/$overallCount_row['count_value'])*5);
    }else{
        $obj_data->overall = round(($shoptake_value12/$overallCount_row)*5);
    }
...
$shop_mapToken = mysqli_query($link,"SELECT `token` FROM `shop_mapping` WHERE `shop_token`='$shop_token' AND `distributor_token`='$dist_token_value'");
...
//shop_count_value
$overallCount  = mysqli_query($link,"SELECT DISTINCT orders.token FROM orders INNER JOIN shop_mapping ON shop_mapping.token =orders.shop_token INNER JOIN units__shop_mapping ON units__shop_mapping.unit_group_token=shop_mapping.unit_token WHERE units__shop_mapping.unit_group_token='$unit_token' AND date(orders.date_time)='$dateonly'  GROUP BY orders.shop_token");
                            
                            $overallCount_row = mysqli_num_rows($overallCount);
//order logs statuscheck 

             $shop_value1 = mysqli_query($link,"SELECT DISTINCT orders.token FROM orders INNER JOIN shop_mapping ON shop_mapping.token =orders.shop_token INNER JOIN units__shop_mapping ON units__shop_mapping.unit_group_token=shop_mapping.unit_token WHERE units__shop_mapping.unit_group_token='$unit_token' AND date(orders.date_time)='$dateonly'AND orders.delivery='Completed' GROUP BY orders.shop_token");
...
//getting balance and paid amount
 ...
 COALESCE(SUM(bill_discount_percentage),0) AS bill_discount_percentage,
