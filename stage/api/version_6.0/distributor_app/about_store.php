...
$shop_detail = mysqli_query($link,"SELECT
				    ...
				    `shop__type`.token as `shop_code`,
				    `coordinates`,
				    `contact_person`
				    FROM `shop` 
                    INNER JOIN `shop__type` ON `shop__type`.`token` = `shop`.`shop_type_code`
				    WHERE `shop`.`token` = '$shop_id' AND `shop`.`distributor_token` = '$distributor_token'");

                    while($shop_row = mysqli_fetch_array($shop_detail)) {
                         $obj = new stdClass;
                         ...
                         $obj->shop_type_code= $shop_row['shop_code'];
