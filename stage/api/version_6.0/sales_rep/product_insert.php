...
VALUES ('$token','$order_number','$currnetDateTime','$order_type','$shop_mapping_token','$distributor_token','$distributor_token','$final_product_item_count',0,'$bill_amount','pending','$unit_shop_count','1','$sale_id')");
 ...
 if($order_insert) {
    ...
    $check_status = mysqli_query($link,"SELECT * FROM `sales__log` WHERE `distributor_token`='$distributor_token' AND `sales_token`='$sale_id' AND  `shop_token` ='$shop_mapping_token' and `date_time` LIKE '$indiaDate%'");
    if(mysqli_num_rows($check_status) >0){

                 $sales_update = mysqli_query($link,"UPDATE `sales__log` SET `status`='Completed'  WHERE `distributor_token` = '$distributor_token' AND `sales_token` = '$sale_id' AND `shop_token`='$shop_mapping_token' AND `date_time` LIKE '$indiaDate%'");
    }
        else
        {
        $sales_log = mysqli_query($link,"INSERT INTO `sales__log`(`date_time`, `distributor_token`, `sales_token`, `shop_token`, `status`) VALUES ('$currnetDateTime','$distributor_token','$sale_id','$shop_mapping_token','Completed')");
        }
}
