...
$sql = mysqli_query($link,"SELECT `name`,`mobile_number`,`deparment_token` from `employees` WHERE `token`='$sales_rep_token'");
...
if($sales_rep_token!=""&& $description!=""){
...
VALUES('$token','$sales_rep_token','$name','$mobile_number','$department_token','$description','$attach_image','0','$current')");
}
$obj = new stdClass;
if($insert_support){
    $obj->status_code=200; 
