...
if($count > 0) {
    ...
    $passwords = hash('sha512', $password4);
    $password_check = mysqli_query($link,"SELECT `id` FROM `shop` WHERE `password`='$passwords' AND `mobile_number`='$mobile'");
  ...
  WHERE
      `mobile_number` = '$mobile' AND `password` = '$passwords' AND shop_mapping.status = '1'");
  ...
  WHERE
      `shop`.`mobile_number` = '$mobile' AND `shop`.`shop_show_status` = 'Active' AND NOT shop_mapping.status = '0'");
       $row = mysqli_fetch_array($query1);
            ...
            $obj_emp->shop_code = $row['shop_code'];
        ...
        $obj->status_code=400; 
        $obj->message='Kindly wait until our team reaches you to assist you in the verification process. Support number - 0413-132266111';
        $obj->title='failure';
    }
}else{
    $obj->status_code=400; 
