...
VALUES ('$token','$order_number','$currnetDateTime','$order_type','$shop_mapping_token','$sale_id','$dist_token_value','$final_product_item_count','$bill_amount','0','pending','$unit_shop_count')");
...
$check_status = mysqli_query($link,"SELECT * FROM `sales__log` WHERE `distributor_token`='$dist_token_value' AND `sales_token`='$sale_id' AND  `shop_token` ='$shop_mapping_token' and `date_time` LIKE '$indiaDate%'");
...
$shopvisted = mysqli_query($link,"INSERT INTO `salesman_shop_visited`(`shop_token`, `employee_token`, `order_token`, `unit_token`, `status`,`date`) VALUES ('$shop_mapping_token','$sale_id','$token','$unit_token','1','$indiaDate')");
