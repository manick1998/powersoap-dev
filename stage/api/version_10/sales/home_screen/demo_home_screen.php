...
$dateonly = $indiaDate;
 ...
 //echo $dateonly;
 $sql = "SELECT * FROM `daily_schedule` WHERE sales_emp_token = $employee_id AND date(date_time) = '$dateonly' ";
...
$obj_data1 = new stdClass;
$obj_data1->cover_today_value=$shoptake_value_12;
$obj_data1->cover_today_outoff=intval($amt_value1['shop_count_value']);
// percentage_value
// discount
...
$obj_data1->today_order = intval($amt_value1['order_value']-($amt_value1['percentage_value']+$amt_value1['discount']));
$obj_data1->today_cover = 0;
$obj_data1->achived_productivity = $shoptake_value_12;
$obj_data1->overall_productivity = intval($amt_value1['shop_count_value']);
$obj_data1->today_collection_amount = round($today_collection_amount);
$obj_data1->overall_item_amount_value = round($overall_item_amt_val);
if($check_countvalue_data > 0){
$obj_data1->productivity = round(($shoptake_value_12/$amt_value1['shop_count_value'])*5);
}else{
$obj_data1->productivity = 0;
}
$obj_data1->unit_token = $amt_value1['unit_token'];
  ...
  LEFT JOIN orders ON orders.shop_token = shop.token AND orders.date_time LIKE '$dateonly%' AND orders.delivery != 'Cancelled' AND custom_daily_schedule.sales_emp_token = 76912841
  ...
  WHERE
    custom_daily_schedule.sales_emp_token = 76912841 AND custom_daily_schedule.schedule_date = 'Tuesday' AND shop.delete_status = '1'  AND unit_customise_mapping.delete_status = '1'
  GROUP BY
     shop.token
  ORDER BY
    shop.id
...
$shop_total_value_count = mysqli_query($link,"SELECT
   ...
   $obj_shop1 = new stdClass;
   $obj_shop1->shop_id = $shop_rows['shop_token'];
   $obj_shop1->shop_name = $shop_rows['name'];

   $obj_shop1->shop_total = round($get_shop_total_value1['billing_amount1'] - $discount_amount_finall_11);
   $obj_shop1->paid_amt = round($shop_rows['paid_amt']);
   $obj_shop1->balance_amt = round(($get_shop_total_value1['billing_amount1']) - ($discount_amount_finall_11 + $get_shop_total_value1['paid_amount1']));
   ...
   $obj_shop1->target_amount =round($get_shop_total_value1['billing_amount1'] - $discount_amount_finall_11);
   $obj_shop1->achived_amount = round($get_shop_total_value1['paid_amount1']);
   $obj_shop1->balance_amount = $get_balance_amt1;
   $obj_shop1->category_name = $shop_rows['type_name'];
   $obj_shop1->delivery = $delivery_value;
   $obj_shop1->items = $get_shop_total_value1['items1'];
   $obj_shop1->address = $shop_rows['address'];
   $obj_shop1->city = $shop_rows['city'];
   $obj_shop1->pincode = $shop_rows['pincode'];
   $obj_shop1->shop_distance = rand(1,10);
   $obj_shop1->units_name = $shop_rows['units_name'];
   array_push($shop_array,$obj_shop1);
                }   
   
   $home_screen1 = new stdClass;
$home_screen1->summary_details = $obj_data1;
$home_screen1->shop_details = $shop_array;
...
//                                 while ($rows = mysqli_fetch_assoc($amount1)) {
                                    
//                                     $new_obj-> shop_token =$rows['shop_token'];
//                                     $new_obj-> name =$rows['name'];
//                                     $new_obj-> address =$rows['address'];
//                                     $new_obj-> city =$rows['city'];
//                                     $new_obj-> pincode =$rows['pincode'];
//                                     $new_obj-> unit_token =$rows['unit_token'];
//                                     $new_obj-> units_name =$rows['units_name'];
//                                     $new_obj-> employees_token =$rows['employees_token'];
//                                     $new_obj-> employees_name =$rows['employees_name'];
//                                     $new_obj-> orders_bill_amt =$rows['bill_amt_val'];
//                                     $new_obj-> orders_paid_amt =$rows['paid_amt_val'];
//                                     $new_obj-> bill_amount =$rows['bill_amount'];
//                                     $new_obj-> paid_amt =$rows['paid_amt'];
//                                     $new_obj-> total_outstanding =$rows['total_outstanding'];
//                                     $new_obj-> distributor_token =$rows['distributor_token'];
//                                     $new_obj-> schedule_date =$rows['schedule_date'];
//                                     $new_obj-> items =$rows['items'];
//                                     $new_obj-> delivery =$rows['delivery'];
//                                     $new_obj-> bill_discount_amount =$rows['bill_discount_amount'];
//                                     $new_obj-> bill_discount_percentage = $rows['bill_discount_percentage'];
//                                     array_push($arr_data,$new_obj);
//                                 }


//                                 $new_obj->code = 200;
//                                 $new_obj->message = "correct";
//                                 $new_obj = $arr_data;
...
//                              $new_obj ->amt_value = $amt_value;    
//                            $new_obj->code = 400;
//                            $new_obj->message = "normul_unit_";  
                        //   array_push($amt_value,$new_obj);                      
}
