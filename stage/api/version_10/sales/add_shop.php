...
//custom schedule
$sql = "SELECT * FROM `custom_daily_schedule` WHERE sales_emp_token ='$employee_id' AND schedule_date = '$dayname'";
$amount = $link->query($sql);
$data1= $amount->num_rows;
...
$obj1 = new stdClass;
    $mobile_numberCheck =mysqli_query($link,"SELECT shop.mobile_number as count,shop.token FROM `shop` INNER JOIN shop_mapping ON shop.token=shop_mapping.shop_token WHERE shop.mobile_number='$contact_number' AND shop_mapping.distributor_token='$distributor_id'");
    $row = mysqli_fetch_array($mobile_numberCheck);
    $mobile_count = $row['count'];
    $shop_token = $row['token'];
    if($data1==0){
    if($mobile_count==0){
        $insert_shop = mysqli_query($link,"INSERT INTO `shop`( `token`, `name`, `date_time`,`retail_code`, `shop_type_code`, `slot`, `mobile_number`,`contact_person`, `join_date`, `license_number`,license_image,  `delete_status`, `address`, `city`, `state_id`, `pincode`, `coordinates`,`created_by`) VALUES ('$token','$shop_name','$date','$retailcode','$shop_type','0%','$contact_number','$contact_name','$date','$licence_number','$upload_url', '1','$address','$city','$state_id','$pincode','$coordinates','$employee_id')");

        $insert_shop_mapping = mysqli_query($link,"INSERT INTO `shop_mapping`(`token`,`shop_token`, `distributor_token`, `unit_token`,`date_time`, `status`) VALUES ('$shop_mapToken','$token','$distributor_id','$units_id','$currnetDateTime','1')");
        $units_mapping = mysqli_query($link,"INSERT INTO `units__shop_mapping`( `unit_group_token`, `distributor_token`, `shop_token`, `delete_status`) VALUES ('$units_id','$distributor_id','$shop_mapToken','1')");
     ...
     $query = mysqli_query($link,"SELECT `shop_token` FROM `shop_mapping` WHERE `distributor_token`='$distributor_id' AND `shop_token`='$shop_token'");
