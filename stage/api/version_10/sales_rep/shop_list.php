...
$sql2 = mysqli_query($link,"SELECT
				...
				INNER JOIN shop ON shop.token = shop_mapping.shop_token AND shop_mapping.status='1'
				LEFT JOIN orders ON orders.shop_token = shop_mapping.token
				INNER JOIN shop__type ON shop__type.token = shop.shop_type_code
				WHERE
				    SR.`token` = $sales_rep_employee_token AND shop_mapping.distributor_token='$distributor_token' AND shop.shop_show_status = 'Active' 
				GROUP BY
				    `shop`.`token`");
		...
		while($row2 = mysqli_fetch_array($sql2)){
			...
			$query = mysqli_query($link,"SELECT `token` FROM `shop_mapping` WHERE `shop_token`='$shop_tken'");
			$row1 = mysqli_fetch_array($query);
			$shopToken = $row1['token'];
            ...
            $getDate = ($get_last_date['last_date']);
