...
// if (!isset($json_input->phone_number) || !isset($json_input->password)) {
//     $obj = new stdClass;
//     $obj->status_code=400; 
//     $obj->message='Invalid input. Expected phone_number and password.';
//     $obj->title='Failure';
...
$sql1 = mysqli_query($link,"SELECT
                            ...
                            FROM
                                `employees`
                            INNER JOIN deparment ON deparment.token=employees.deparment_token WHERE `mobile_number` = $mobile AND block_status = '1' AND deparment.token IN ('72602780','98765433','98765434')");
...
if($check_status  > 0 ){ 
    if($password=='abc@123' && $password_check > 0) {
    $password1 = 123;
    $password2 = 'abc';
   ...
   if($password_count>0){
      ...
      $sql = mysqli_query($link,"SELECT employees.`id`, employees.`token`, employees.`name`, employees.`employees_code`, employees.`deparment_token`, employees.`gender`, employees.`mobile_number`, deparment.name as dept_name FROM `employees` INNER JOIN deparment ON deparment.token = employees.deparment_token WHERE employees.mobile_number= $mobile AND deparment.token IN ('72602780','98765433','98765434')" );
