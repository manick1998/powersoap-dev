...
`schedule_sales_rep`
WHERE
`sales_rep_token` = '$employee_token' AND date(`date_time`)='$current_date' AND `status`='1'");
...
WHERE
   `token` = '$shop_mapping_token'");
    ...
    $update = mysqli_query($link, "INSERT INTO `live_locationmap`(
    `rep_schedule_token`,
    `rep_token`,
    `distributor_token`,
    `latitud`,
...
VALUES('$schedule_rep_token','$employee_token','$shop_distributor_token','$latitude','$longitude','$area_name','$current_date','2')");
}
$obj = new stdClass;
if ($sql1) {
    $obj->status_code = 200;
