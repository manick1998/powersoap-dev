...
if($order_row['units'] == 'Box'){
    $order_obj->amount = number_format($order_row['amount']*$box_value);
}else{
    $order_obj->amount = number_format($order_row['amount']);
}
  $order_obj->misc_price = number_format((float)$order_row['gst'], 2, '.', '');
