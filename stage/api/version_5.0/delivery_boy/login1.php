...
$obj = new stdClass;
// $sql1 = mysqli_query($link,"SELECT 1 FROM `employees` inner JOIN deparment WHERE `mobile_number`= $mobile AND deparment.token=93402780 AND block_status='1'");

$sql1 = mysqli_query($link,"SELECT
                                *
                            FROM
                                `employees`
                            INNER JOIN deparment ON deparment.token=employees.deparment_token WHERE `mobile_number` = $mobile AND block_status = '1' AND deparment.token= '93402780'");
