...
$obj1 = new stdClass;
    $mobile_numberCheck =mysqli_query($link,"SELECT shop.mobile_number as count,shop.token FROM `shop` INNER JOIN shop_mapping ON shop.token=shop_mapping.shop_token WHERE mobile_number='$contact_number' AND shop_mapping.distributor_token!='$distributor_id'");
    ...
    if($mobile_count ==0){
        ...
        $insert_shop_mapping = mysqli_query($link,"INSERT INTO `shop_mapping`(`token`,`shop_token`, `distributor_token`, `date_time`, `status`) VALUES ('$shop_mapToken','$token','$distributor_id','$currnetDateTime','1')");
