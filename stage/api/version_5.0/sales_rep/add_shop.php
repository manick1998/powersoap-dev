...
$mobile_numberCheck =mysqli_query($link,"SELECT mobile_number FROM `shop`  WHERE mobile_number='$contact_number'");
...
if($mobile_count ==0){
    ...
    {
    $shop_mapToken = genToken('token','shop_mapping');
    $insert_shop_mapping = mysqli_query($link,"INSERT INTO `shop_mapping`(`token`,`shop_token`, `distributor_token`, `date_time`, `status`) VALUES ('$shop_mapToken','$token','$distributor','$currnetDateTime','0')");
   ...
   foreach($distributor_token as $distributor)
   {
    ...
    if($count==0){
        $shop_mapToken = genToken('token','shop_mapping');
     $insert_shop_mapping = mysqli_query($link,"INSERT INTO `shop_mapping`(`token`,`shop_token`, `distributor_token`, `date_time`, `status`) VALUES ('$shop_mapToken','$token1','$distributor','$currnetDateTime','0')");
     $shop_outstanding_amt = mysqli_query($link,"INSERT INTO `shop__outstanding`( `date_time`, `shop_token`, `bill_amount`, `paid_amt`, `total_outstanding`, `receiver_token`) VALUES ('$date','$shop_mapToken','0','0','0','0')");
