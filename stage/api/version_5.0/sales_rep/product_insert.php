...
$order_number = "ORD-R".$token;
   ...
   $getShopDistributor = mysqli_query($link,"SELECT `shop_mapping`.`distributor_token` FROM `shop_mapping` INNER JOIN `schedule_sales_rep` ON `shop_mapping`.`distributor_token`=`schedule_sales_rep`.`distributor_token` WHERE schedule_sales_rep.schedule_date='$currnetDate' AND `shop_mapping`.`shop_token`='$shop_token'");
...
VALUES ('$token','$order_number','$currnetDateTime','$order_type','$shop_mapping_token','$shop_distributor_token','$shop_distributor_token','$final_product_item_count',0,'$bill_amount','pending','$unit_shop_count','1','$sale_id')");
 ...
 if($order_insert) {
    ...
    $check_status = mysqli_query($link,"SELECT * FROM `sales__log` WHERE `distributor_token`='$shop_distributor_token' AND `sales_token`='$sale_id' AND  `shop_token` ='$shop_mapping_token' and `date_time` LIKE '$indiaDate%'");
    if(mysqli_num_rows($check_status) >0){

                 $sales_update = mysqli_query($link,"UPDATE `sales__log` SET `status`='Completed'  WHERE `distributor_token` = '$shop_distributor_token' AND `sales_token` = '$sale_id' AND `shop_token`='$shop_mapping_token' AND `date_time` LIKE '$indiaDate%'");
    }
        else
        {
        $sales_log = mysqli_query($link,"INSERT INTO `sales__log`(`date_time`, `distributor_token`, `sales_token`, `shop_token`, `status`) VALUES ('$currnetDateTime','$shop_distributor_token','$sale_id','$shop_mapping_token','Completed')");
        }
}


$bill_amount=0;
$product_dist_count=0;
foreach( $product_array as $key => $data) {
     ...
     if($is_free == false){             
         ...
         $select_dist_stock = mysqli_query($link,"SELECT `stock_in_hand`,`sold_pieces` FROM `stock__distributor` WHERE `product_token`='$product_token' AND`employee_token`= '$shop_distributor_token'");
         ...
         if($units =='Box'){
            ...
            $stockUpdate = mysqli_query($link,"UPDATE `stock__distributor` SET `stock_in_hand`='$newcount_stock' WHERE `product_token`='$product_token' AND `employee_token`='$shop_distributor_token'");
         } 
         else {
            $disrtributor_product_count =$order_qty;
            ...
            $stockUpdate = mysqli_query($link,"UPDATE `stock__distributor` SET `stock_in_hand`='$new_box', `sold_pieces`='$balanace_pieces' WHERE `product_token`='$product_token' AND `employee_token`='$shop_distributor_token'");
         }
                ...
                if($is_scheme){
                    ...
                    // qutatity cal
                     ...
                     $select_dist_stock = mysqli_query($link,"SELECT `stock_in_hand` FROM `stock__distributor` WHERE `product_token`='$product_token' AND`employee_token`= '$shop_distributor_token'");
         $selectrow_dist = mysqli_fetch_array($select_dist_stock);
         $old_stock_count = $selectrow_dist['stock_in_hand']; 
