...
$order_insert = mysqli_query($link,"INSERT INTO `orders`( `token`,`order_number`, `date_time`, `order_type`, `shop_token`, `employee_token`, `items`, `billing_amount`,`delivery`,                  `delivery_emp_token`, `approved_on`, `delivered_on`) 
                        VALUES ('$token','$order_number', '$currnetDateTime','$order_type','$shop_token','$sale_id','$item_count','0','Completed','$sale_id','$currnetDateTime','$currnetDateTime')");
...
foreach( $product_array as $key => $data) {
     ...
     $order_product = mysqli_query($link,"INSERT INTO `orders__items`( `order_token`, `product_token`, `price_per_unit` , `piece_count`, `misc_price`, `quantity`, `units`, `date_time`) VALUES 
 ('$token','$product_token','0','$piece_count','0','$order_qty','$units','$currnetDateTime')");
...
if($check) {
    ...
    $check_status = mysqli_query($link,"SELECT * FROM `sales__log` WHERE `distributor_token`='$dist_token_value' AND `sales_token`='$sale_id' AND  `shop_token` ='$shop_token'");
