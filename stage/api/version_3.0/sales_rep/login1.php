...
if($check_status  > 0) {
    ...
    $update = mysqli_query($link,"UPDATE `employees` SET `password`='$password' WHERE `mobile_number`='$mobile'");
    $password1=$json_input->passwords;
    $password4 = hash('sha512', $password1);
   ...
   if($password_count>0){
      $query = mysqli_query($link,"SELECT `id` FROM `employees` WHERE `mobile_number`='$mobile' AND `password`='$password' AND `block_status` ='1'");
      $result = mysqli_num_rows($query);
      $sql = mysqli_query($link,"SELECT employees.`id`, employees.`token`, employees.`name`, employees.`employees_code`, employees.`deparment_token`, employees.`gender`, employees.`mobile_number`, deparment.name as dept_name FROM `employees` INNER JOIN deparment ON deparment.token = employees.deparment_token WHERE employees.mobile_number= $mobile AND deparment.token= '72602780'" );
