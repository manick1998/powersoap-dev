...
$order_details = mysqli_query($link,"SELECT
  ...
  while($order_row = mysqli_fetch_array($order_details)) {
        ...
        // $order_obj->amount = round($order_row['amount']*$box_value);
        $amount_val1 = $order_row['price_per_unit']*$order_row['qty'] - $discount_amount_finall;
        $order_obj->amount =number_format($amount_val1, 2, '.', '');
        // }else{
        //     $order_obj->amount = round($order_row['amount']);
        // }
        $order_obj->misc_price = round($order_row['gst']);
    ...
    $order_total = mysqli_query($link,"SELECT
            ...
            // $amt_obj->gst = number_format((float)$order_mount['gst'], 2, '.', '');
            // $value = number_format((float)$order_mount['billing_amount'] -$order_mount['gst'], 2, '.', '') ;
            // $amt_obj->pro_amount = number_format((float)$value, 2, '.', '');
            $amt_obj->gst = round($order_mount['gst']);
            $value = round($order_mount['billing_amount'] -$order_mount['gst']) ;
            $amt_obj->pro_amount = round($value);
            // discount_value
            ...
            // $amt_obj->discount_new_value = round(($order_mount['overall']) - ($order_mount['billing_amount']));
            // 1%discount
