...
$order->distributor_token = $distributor_token;
...
<div style="width: 1000px;margin: 0 auto;table-layout: fixed;">
    <table cellpadding="8" cellspacing="0" style="border-collapse: collapse;width: 100%;border:2px solid #000;text-align: left;background-color: #fff;font-size: 12px; font-family: sans-serif;">
        <tr>
            <td colspan="7" style="border: 1px solid #000;font-size: 12px;margin: 0;white-space: nowrap;"><b>' . $stmt->distributor_name . '</b><br/>
                <span>' . $stmt->distributor_address . '</span><br><span><b>GSTIN:</b>' . $stmt->distributor_license . '</span>
            </td>
            <td colspan="7" style="border: 1px solid #000;font-size: 12px;margin: 0;white-space: nowrap;"><span><b>Tax Invoice</b></span><br/>
                <span><b>Phone no :</b> ' . $stmt->shop_mobile_number . '<br/><b>Email :</b> ' . $stmt->email . '</span>
            </td>
        </tr>
        <tr>
            <td colspan="7" style="border: 1px solid #000;font-size: 12px;margin: 0;white-space: nowrap;"><span><b>Buyer :</b>' . $stmt->shop_name . '</span><br/>
            ...
            <td colspan="7" style="border: 1px solid #000;text-align: left;font-size: 12px;vertical-align: text-top;"><span><b>No. :</b> ' . $order_id . ' <br/><b>Date :</b> ' . $indiaDateFormat . '<br/></span>
            </td>
        </tr>
        ...
        <tr>
            ...
            <th colspan="2" style="border: 1px solid #000;text-align: center;font-size: 10px;margin: 0;color:#000;">Rate <br/>(Bef.Tax)</th>
            ...
            <th style="border: 1px solid #000;text-align: center;font-size: 10px;margin: 0;color:#000;">Net Rate</th>
            
            <th style="border: 1px solid #000;text-align: center;font-size: 10px;margin: 0;color:#000;white-space: nowrap;">Net Amount</th>
...
while ($row = $stmt1->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($row['is_free'] == '0') {
        $html .= '<td  colspan="2" style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;">' . round($row['price_per_unit'] / 1.18) . '</td>
                    ...
                    <td style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;">' . ($row['price_per_unit']) . '</td>';
        if ($row['units'] == "Box") {
            $totalBox += $row['quantity'];
            $html .= '<td style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;">' . indCurrencyFormatComma($totalAmtAfterDisc) . '</td>';
        } else {
            $totalPcs += $row['quantity'];
            $html .= '<td style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;">' . indCurrencyFormatComma($totalAmtAfterDisc) . '</td>';
        }
    } else {
        $html .= '<td style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;">-</td>

                <td style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;">-</td>
                ...
                <td style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;">-</td>
                <td style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;">Free</td>';
        if ($row['units'] == "Box") {
            $totalBox += $row['quantity'];
        } else {
            $totalPcs += $row['quantity'];
...
$html .= '<tr>
            ...
            <td colspan="8"  style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;"></td>
            
            <td style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;"><span style="padding-top: 5px;border-top:2px solid #000;"><b>' . indCurrencyFormatComma($newTotalAmount) . '</b></span></td>
        </tr>';
$html .= '<tr>
            ...
            <td style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;">' . indCurrencyFormatComma(round($newTotalAmount)) . '</td>
        </tr>';
$html .= '<tr>
            <th style="padding: 12px;border-right: 1px solid #000;border-left: 1px solid #000;"></th>
            <th style="padding: 12px;border:1px solid #000;font-size: 8px;color:#333;">HSN</th>
            ...
            <th colspan="2" style="padding: 12px;text-align: center;border:1px solid #000;font-size: 8px;color:#333;">CGST</th>
            <th colspan="2" style="padding: 12px;text-align: center;border:1px solid #000;font-size: 8px;color:#333;">SGST</th>
        </tr>';
foreach ($ob as $key => $obj) {
    $html .= '<tr>
            <td style="padding: 10px;border-right: 1px solid #000;border-left: 1px solid #000;"></td>
            <td style="padding: 10px;border:1px solid #000;font-size: 8px;color:#333;">
            ' . $key . '
            </td>
            <td style="padding: 10px;text-align: center;border:1px solid #000;font-size: 8px;color:#333;">
                18%
            </td>
            <td colspan="2" style="padding: 10px;text-align: center;border:1px solid #000;font-size: 8px;color:#333;">
            ...
            <td colspan="5" style="padding: 10px;border-top:1px solid #000;border-bottom:1px solid #000;border-right:1px solid #000;"></td>
        </tr>';
    $hsnTotal += $obj / 1.18;
    $gstTotal += number_format(($obj - ($obj / 1.18)) / 2, 2, '.', '');
}
$html .= '<tr>
            <td style="font-size: 8px;padding: 10px;border-top:2px solid #000;border:1px solid #000;"></td>
            <td colspan="2" style="font-size: 8px;padding: 10px;border-top:1px solid #000;border-bottom:1px solid #000;">TOTAL:</td>
            ...
            <td style="text-align: center;font-size: 8px;padding: 10px;border-top:1px solid #000;border-bottom:1px solid #000;">' . $gstTotal . '</td>
            <td style="font-size: 8px;padding: 10px;border-top:1px solid #000;border-bottom:1px solid #000;"></td>
            <td style="text-align: center;font-size: 8px;padding: 10px;border-top:1px solid #000;border-bottom:1px solid #000;">' . $gstTotal . '</td>
            <td colspan="5" style="font-size: 8px;padding: 10px;border-top:1px solid #000;border-bottom:1px solid #000;border-right:1px solid #000;"></td>
        </tr>';


$html .= '<tr>
            <td style="padding: 10px;border: 1px solid #000;"> </td>
            <td colspan="5" style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;"></td>
            <td colspan="2" style="border: 1px solid #000;font-size: 8px;margin: 0;padding:0 15px;"><b>TOTAL :</b></td>';
if ($stmt->bill_discount_amount == 0 && $stmt->bill_discount_percentage == 0) {
    ...
    $html .= '<td colspan="3" style="border: 1px solid #000;text-align: right;font-size: 8px;margin: 0;padding:0 15px;"><b>Rs: <span>' . $roundTotal . '</span></b></td>';
    ...
    $html .= '<td colspan="3" style="border: 1px solid #000;text-align: right;font-size: 8px;margin: 0;padding:0 10px;"><b>Rs: <span>' . (round($newTotalAmount)) . '</span></b></td>';
    $numbertoWord = numbertoword(round($roundtotalDiscountAmount));
}
$html .= '</tr>
        <tr>
            <td colspan="7" style="border-right: 1px solid #000;font-size: 8px;">' . $numbertoWord . '</td>
            <td colspan="7" style="border-right: 1px solid #000;"></td>
        </tr>
        <tr>
            <td colspan="7" style="font-size: 8px;line-height: 22px;margin: 0;padding:0;border-bottom: 1px solid #000;border-right: 1px solid #000;">Due : <span>' . $indiaDateFormat . '</span></td> 
        
            <td colspan="7" style="text-align: center;font-size: 8px;line-height: 22px;margin: 0;padding:0;border-right: 1px solid #000;border-bottom: 1px solid #000;"><br/><b>Signature</b></td>
        </tr>
    </table>
</div>
</body>
