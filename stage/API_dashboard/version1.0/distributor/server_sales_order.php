...
if ($_GET['v_id'] == $verification_code) {
    ...
    $totalRecords = $order->orderCheckCountSal();
    ## filet query values
    ...
    ## filer count check
    if ($dateQuery != " ") {
        $stmt = $order->orderCheckFilterSal();
        $totalRecordwithFilter = $order->orderCheckFilterSal();
    }
    if ($searchQuery != " ") {
        $totalRecordwithFilter = $order->orderCheckSearchSal();
    }
