...
if (isset($input_data->dashboard_code) && $input_data->dashboard_code == $verification_code) {
    ...
    $stmt = $stockOrder->getDistributorProductDetail();
    $num = $stmt->rowCount();
    $obj = new stdClass;
    if($num){
        $array = $stockOrder->viewGetDistributorProductDetail($stmt);
        $obj->status_code = 200;
        $obj->header = "Success";
        $obj->message = "Stock In hand Detail";
        $obj->data = $array;
    }else{
        $obj->status_code = 400;
        $obj->header = "Oops";
        $obj->message = "No Stock In hand Detail";
    }
