    ...
    function readLeaveMangement($stmt)
    {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            if ($row['leave_status'] == "Pending") {
                ...
                $status  = '<button class="tb-btn greenbtn">' . $row['leave_status'] . '</button>';
            } else {
                $status  = '<button class="tb-btn red">' . $row['leave_status'] . '</button>';
            }
            $data[] = array(
                "salesname" => $row["name"],
...
WHERE
    ...
    {
        $query = "SELECT
            live_location.rep_schedule_token,
            live_location.rep_token,
            employees.employee_image AS image,
            employees.name AS rep_name,
            area.area_name AS schedule_area_name,
            live_location.area_location,
            live_location.area_name AS live_area_name,
            live_location.latitud,
            live_location.langitud,
            MAX(live_location.date_time) AS last_time
        FROM
            `live_location`
        INNER JOIN employees ON employees.token = live_location.rep_token
        LEFT JOIN schedule_sales_rep ON schedule_sales_rep.rep_schedule_token = live_location.rep_schedule_token
        LEFT JOIN area ON area.area_token = schedule_sales_rep.area_token
        WHERE
            DATE(live_location.date_time) = DATE('$date')
    ...
    {
        $query = "SELECT * FROM `live_location` WHERE rep_schedule_token = ? AND rep_token = ?";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $this->rep_schedule_token);
    ...
    function particulare_state_salesrep_latlang($date, $beforemin)
    {
        ...
        WHERE
            schedule_sales_rep.state_token = ?
            AND DATE(live_location.date_time) = DATE('$date')
            AND employees.deparment_token = '72602780'
            AND employees.block_status = '1'
            AND employees.delete_status = '1'
        GROUP BY
