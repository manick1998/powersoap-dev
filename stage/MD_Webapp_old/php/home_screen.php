...
$detail = new stdClass;
// order_taken
$order_item = mysqli_query($link,"SELECT COUNT(`id`) as order_taken FROM `orders` WHERE `order_type` in ('Distributor Order') AND `delivery` = 'Completed'");
...
//yearly
    ...
    $detail->year_income = indCurrencyFormatComma(round($get_year['year']));
     ...
     //monthly
     $month_data = mysqli_query($link,"SELECT COALESCE(SUM(`billing_amount`),0)AS `month` FROM `orders` WHERE `delivery`='Completed' AND `order_type`='Distributor Order'");
     ...
     $detail->month_income = indCurrencyFormatComma(round($get_order_taken['month']/$month));
          ...
          $detail->month=indCurrencyFormatComma($month);
