                ...
                <div class="tab-pane active" id="state1">
                    <section class="bg-white brad-4 full-height" >
                        <div class="product_header_container">
                            ...
                            <h1 class="header_main"><span class="twoinspace">Schedule Log</span></h1>
                            </div>
                        </div>
                        <!-- Nav tabs -->
                        ...
                        <div class="modal-inner-body">
                            <div class="banner-option-box">
                                <div class="popup-image-box">
                                    ...
                                    <div class="form-control region_token_box">
                                        <p for="offer_division" class="input-field ">State</p>
                                        <input class="input-field" id="state_name" value="">
                                    </div>
                                    <div class="form-control region_token_box" id="selectregionrep">
                                        <p for="offer_division" class="input-field ">Region</p>
                                        <input class="input-field" id="region_name" value="">
                                    </div>
                                    <div class="form-control region_token_box" >
                                        <p for="offer_division" class="input-field ">Area Distributor</p>
                                        <ul id="area_distributor"></ul>
                                    </div>
                                    <div class="form-control region_token_box" id="selectedareaRep">
                                        <p for="offer_division" class="input-field ">Area</p>
                                        <!-- <input class="input-field" id="rep_area" value=""> -->
                                        <ul id="rep_area"></ul>
                                    </div>    
                                </div>
                            </div>
        ...
        <script>
            ...
            $.ajax({
                ...
                data.data.forEach(function(item,index){
                    ...
                    html += `<td><a id="editSales" data-toggle="modal" data-target="#oldmod" data-date="${item.date}" data-token = "${item.sales_rep_token}" ><span style="color:#00b9f5;cursor: pointer;">View</span></td>`
                    if(item.region_token!=''){
                    html += `<td><a id="newSales" data-toggle="modal" data-target="#newmod" data-date="${item.date}" data-token = "${item.sales_rep_token}"><span style="color:#00b9f5;cursor: pointer;">View</span></td>`
                    }else{
                        html +=`<td>-</td>`
                    }
            ...
            $.ajax({
                   ...
                   // console.log(datas);
                    ...
                    let value1 = datas.datas;
                    let value2= datas.dataas;
                    console.log(value1);
                    $("#edit_state_name").val(value.state_name);
                    $("#edit_region_name").val(value.region_name);
                    var html='';
                    for(var key1 in value1){
                       html += '<li>'+ value1[key1] +'</li>';
                    }
                    $('#edit_rep_area').html(html);
                    var html1='';
                    for(var key2 in value2){
                       html1 += '<li>'+ value2[key2] +'</li>';
                    }
                    $('#edit_area_distributor').html(html1);
                })
        });

        $('body').on('click','#newSales',function(){
            let salesReoToken = $(this).attr('data-token');
            let date = $(this).attr('data-date');
            let datas = {
                salesReoToken:salesReoToken,
            ...
            $.ajax({
                    ...
                    $("#region_name").val(value.region_name);
                    var html='';
                    for(var key1 in value1){
                       html += '<li>'+ value1[key1] +'</li>';
                    }
