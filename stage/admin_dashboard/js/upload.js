...
//upload scheme
function file_upload_scheme(file_id,view_id,exist_image){
    var fileUpload = document.getElementById(file_id+"_upload");
    var files = !!fileUpload.files ? fileUpload.files : [];
    var regex = new RegExp("([a-zA-Z0-9\s_\\.\-:])+(.jpg|.png|.gif|.jpeg)$");
    if (regex.test(files[0].type)) {
...
function makeid(length) {
 ...
 function file_upload_image_or_pdf(file_id,view_id,exist_image){
    var fileUpload = document.getElementById(file_id+"_upload");
    var file = !!fileUpload.files[0] ? fileUpload.files[0] : [];
    if (file.type != "application/pdf") {
       if (typeof (fileUpload.files) != "undefined") {
            ...
            reader.onload = function (e) {
                ...
                image.src = e.target.result;
                image.onload = function () {
                    $('#'+file_id+'_upload').next(".reupload").remove();
                    $('#'+view_id+'_pdf').hide();
                    $('#'+view_id+'_pdf').attr("src","");
                    $('#'+view_id+'_image').show();
                    $('#'+view_id+'_close').show();
                    $('#'+view_id+'_image').attr('src', e.target.result);
                    $('#'+file_id+'_valid').val("true");
                    $('#'+file_id+'_upload').after('<p class="reupload">Reupload</p>');
        ...
        var fileReader = new FileReader(); 
            ...
            $('#'+file_id+'_upload').next(".reupload").remove();
            $('#'+view_id+'_image').hide();
            $('#'+view_id+'_image').attr("src","");
            $('#'+view_id+'_pdf').attr("src", e.target.result);
            $('#'+file_id+'_valid').val("true");
            $('#'+view_id+'_pdf').show();
            $('#'+view_id+'_close').show();
            $('#'+file_id+'_upload').after('<p class="reupload">Reupload</p>');
...
function file_removes(file_id,view_id,exist_image){
    $('#'+file_id+'_upload').val('');
    $('#'+view_id+'_close').hide();
    $('#'+view_id+'_pdf').hide();
    $('#'+view_id+'_pdf').attr("src", '');
    $('#'+file_id+'_valid').val("");
    $('#'+file_id+'_upload').next(".reupload").remove();
    $('#'+view_id+'_image').attr("src",exist_image);
    $('#'+view_id+'_image').show();
            
}
function edit_file_removes(file_id,view_id,exist_image){
    $('#'+file_id+'_upload').val('');
    $('#'+view_id+'_close').hide();
    $('#'+view_id+'_pdf').hide();
    $('#'+view_id+'_pdf').attr("src", '');
