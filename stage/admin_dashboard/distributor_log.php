        ...
        #divLargerImage
{
    ...
    <body>
                ...
                <div class="tab-pane active" id="state1">
                    <section class="bg-white brad-4 full-height" >
                        <div class="product_header_container">
                            ...
                            <h1 class="header_main"><span class="twoinspace hide">Distributor Log</span></h1>
                            </div>
                        </div>
                        <!-- Nav tabs -->
        ...
        <div class="modal fade" id="oldmod" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
document.querySelectorAll('.my-lightbox-toggle').forEach((el) => el.addEventListener('click', (e) => {
	...
	lightbox.show();
        ...
        <script>
            ...
            $.ajax({
                ...
                data.data.forEach(function(item,index){
                    html += `<tr>`
                    html += `<td>${item.old_name}</td>`
                    html += `<td>${item.new_name==" "?"-":item.new_name}</td>`
                    html += `<td>${item.old_mobile}</td>`
                    html += `<td>${item.new_mobile=="0"?"-":item.new_mobile}</td>`
                    html += `<td>${item.old_email}</td>`
                    html += `<td>${item.new_email==" "?"-":item.new_email}</td>`
                    html += `<td>${item.old_license}</td>`
                    html += `<td>${item.new_license == " "?"-":item.new_license}</td>`
                    if (item.status_code == 3) {
                        html += `<td><a id="new_division" data-distributor_token="${item.distributor_token}" data-toggle="modal" data-target="#newmod"><span style="color:#00b9f5;cursor: pointer;">View</span></td>`
                    }else{
                        html += `<td><a id="old_division" data-distributor_token="${item.distributor_token}" data-toggle="modal" data-target="#oldmod"><span style="color:#00b9f5;cursor: pointer;">View</span></td>`
                    }
                    if(item.new_name!=''){
                    html += `<td><a id="new_division" data-distributor_token="${item.distributor_token}" data-toggle="modal" data-target="#newmod"><span style="color:#00b9f5;cursor: pointer;">View</span></td>`
                    }else{
                    ...
                    if (item.block_status == 1) {
                        html += `<td><butto style="color: #fff; padding: 6px 18px;border-radius: 4px;border: none;background: #40ba07;">Active</butto></td>`
                    }else{
                        html += `<td><butto style=" color: #fff; padding: 6px 18px;border-radius: 4px;border: none;background: #e30909;">Deactive</butto></td>`
                    }
        ...
        $(document).on('click','#new_division',function(){
                var distributor_token = $(this).attr('data-distributor_token');
                var data = {
                distributor_token:distributor_token,
                type :'Distributor_division_log1'
            }
            var json_data = JSON.stringify(data);
            console.log('json_data',json_data);
            $.ajax({
